# **Ce fichier contient des notes en tout genre sur le projets.**

### Configuration Materiel:

Il serait possible d'avoir cela:

* Carte `Arduino UNO Q`
  - **Utilité:** Elle gère a la foit les calcules d'entrainement et de detection de visage en python, et la gestion des moteurs etc.
* Camera USB
  - **Specs:** 420p, 30fps.
  - **Utilité:** Elle permet de faire l'analyse faciale des utilisateurs, une basse qualité (420p) permet d'avoir quelque chose d'utilisable et rapide.
* Hub USB avec *Allimentation*
  - **Utilité:** Ce hub permet de brancher a la foit la Camera et d'allimenter la carte.
* Allimentation: Batterie externe
  - **Utilité:** Permet d'allimenter la carte arduino sans problème pour la caméra et les moteurs.
* LED
  - **Utilité:** Pourquoi pas avoir une led qui eclaire l'utilisateur pour le scan du visage, pour éviter des problème d'eclairage etc?

* Aimants
  - **Utilité:** Pour le système de porte a l'arrière.
  - **Detail:** Fournis par le professeur (2 Aimants de 0.5 cm de diamètre et 0,2 cm d'épaisseur).

* Roue MultiDirectionnelle
  - **Utilité:** Pour aider la partie haute du caroussel à tourner beaucoup plus simplement



# PA TOUCHEUX !!!!!!!! :( :( :(
# U   =  R   *   I
